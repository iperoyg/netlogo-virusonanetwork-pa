...
[
  ...
  dead?               ;; if true, the turtle can't be infected, but keeps infectious
  virus-check-timer   ;; number of ticks since this turtle's last virus-check
  infected-ticks      ;; number od tickes sice this turtle was infected
  resistance
  virus-force
]
...
to make-node [old-node]
  ...
  [
    ...
    if old-node != nobody
      [
        ifelse enable-directed-links
        [create-link-from old-node [ set color green ]]
...
"dead" 1.0 0 -6459832 true "" "plot (count turtles with [dead?]) / (count turtles) * 100"
...
468
Susceptible Nodes
count turtles with [not dead? and not infected? and not resistant?]
17
1
11
